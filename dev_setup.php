...
function dev_generate_config(array $dbConfig): array {
    ...
    $config = [
        ...
        'security' => [
            ...
            // Only ever set true here, by the local dev installer. Never add
            // this key to config.example.php or a production config — it
            // makes require_admin() auto-log-in as the seeded dev admin
            // instead of redirecting to /admin/login. See auth.php.
            'dev_mode' => true,
        ],
...
/**
 * Seed a real admin_users row so dev_mode has an actual account to log in
 * as, rather than faking session state for a user that doesn't exist.
 * Password is set (not left blank) so switching dev_mode off later still
 * leaves a working login.
 */
function dev_seed_admin_account(PDO $pdo): void {
    ...
    $pdo->prepare(
        'INSERT INTO admin_users (email, password_hash, totp_enabled, created_at)
         VALUES (?, ?, 0, NOW())'
    )->execute([$email, $hash]);

    echo "[✓] Seeded dev admin: {$email} / {$password} (only used if dev_mode is off)\n";
}
